    ...
    try:
        if event["RequestType"] == "Create" or event["RequestType"] == "Update":
            ...
            activation_url = re.search(
                r"ACTIVATIONURL='(.+?)'", response_str).group(1)
            manager_url = re.search(
                r"MANAGERURL='(.+?)'", response_str).group(1)
            tenant_id = re.search(
                r'tenantID:(\w{8}-\w{4}-\w{4}-\w{4}-\w{12})', response_str).group(1)
            token = re.search(r'token:([\w-]+)', response_str).group(1)
            ou_ids_str = ', '.join(all_ou_ids)
            ou_ids_str = ou_ids_str.replace(
                '[', '').replace(']', '').replace("'", "")

            response_data = {"ActivationURL": activation_url, "ManagerURL": manager_url,
                             "TenantID": tenant_id, "Token": token, "OrganizationalUnits": str(ou_ids_str)}
